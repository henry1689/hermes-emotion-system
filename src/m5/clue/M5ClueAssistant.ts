...
export class M5ClueAssistant {
  ...
  private conversationBuffer: Array<{ role: 'user' | 'ai'; text: string }> = [];

  constructor(m8Engine: M8Engine) {
  ...
  async processUserInput(config: ClueQuestionConfig): Promise<ClueQuestionResult> {
    ...
    if (lastWasQuestion || lastWasLowConfidence) {
      ...
      if (searchResult.entries.length > 0 && searchResult.entries[0].composite_score >= 0.6) {
        this.conversationBuffer.push({ role: 'user', text: originalQuery });
        return {
          needsQuestion: false,
          searchResult,
          isReady: true,
      ...
      this.conversationBuffer.push({ role: 'user', text: originalQuery });
      this.conversationBuffer.push({ role: 'ai', text: followUp });
      return {
        needsQuestion: true,
    ...
    if (isVagueQuery(originalQuery)) {
      ...
      this.conversationBuffer.push({ role: 'user', text: originalQuery });
      this.conversationBuffer.push({ role: 'ai', text: question });
      return {
        needsQuestion: true,
  ...
  private async searchWithClue(params: {
    ...
    return this.m8Engine.matchByClue({
  ...
  /**
   * 提取原始 Query（取对话缓存中第一个用户消息）
   */
  private extractOriginalQuery(): string {
    return this.conversationBuffer.find((m) => m.role === 'user')?.text ?? '';
  }
